...
class _VideoPlayerWidgetState extends State<VideoPlayerWidget> {
  ...
  Future<void> _loadCaptions() async {
    ...
    try {
      final response = await Get.find<GetConnect>().get(widget.video.captionUrl!);
  ...
  Widget _buildCaptionButton(BuildContext context) {
    ...
    // Only show CC button if video has captions
    if (!widget.video.hasCaptions) {
    ...
    return Positioned(
      ...
      right: 72, // Position to the left of mute button
      child: Obx(() {
        final showCaptions = controller.showCaptions.value;
        return Container(
          decoration: BoxDecoration(
            color: Colors.black.withOpacity(0.5),
            shape: BoxShape.circle,
            border: Border.all(
              color: showCaptions ? Colors.white.withOpacity(0.3) : Colors.white.withOpacity(0.15),
              width: 1,
            ),
            boxShadow: [
              BoxShadow(
