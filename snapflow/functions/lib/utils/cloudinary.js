...
exports.cloudinaryNotificationUrlParam = exports.cloudinaryPrivateDelivery = exports.cloudinaryApiSecret = exports.cloudinaryApiKey = exports.cloudinaryCloudName = void 0;
exports.configureCloudinary = configureCloudinary;
exports.resolveCloudinaryDeliveryTypeForPrivacy = resolveCloudinaryDeliveryTypeForPrivacy;
exports.inferCloudinaryDeliveryTypeFromUrl = inferCloudinaryDeliveryTypeFromUrl;
...
exports.cloudinaryApiKey = (0, params_1.defineString)('CLOUDINARY_API_KEY');
exports.cloudinaryApiSecret = (0, params_1.defineString)('CLOUDINARY_API_SECRET');
exports.cloudinaryPrivateDelivery = (0, params_1.defineString)('CLOUDINARY_PRIVATE_DELIVERY', {
    default: 'upload',
    description: "Delivery type for privacy='private'. Use 'upload' for reliable HLS playback; 'authenticated' can require segment-level signing depending on player/CDN.",
});
exports.cloudinaryNotificationUrlParam = (0, params_1.defineString)('CLOUDINARY_NOTIFICATION_URL', {
    default: '',
    description: 'Absolute HTTPS endpoint Cloudinary should call for auto_transcription events.',
});
let configured = false;
const firebaseConfig = process.env.FIREBASE_CONFIG ? JSON.parse(process.env.FIREBASE_CONFIG) : {};
function resolveCloudinaryNotificationUrl() {
    const configuredUrl = exports.cloudinaryNotificationUrlParam.value()?.trim();
    if (configuredUrl) {
    ...
    if (!projectId) {
        throw new Error('CLOUDINARY_NOTIFICATION_URL not set and project ID is unavailable. Set the env parameter in .env.');
    }
...
function configureCloudinary() {
    ...
    const cloudName = exports.cloudinaryCloudName.value();
    const apiKey = exports.cloudinaryApiKey.value();
    const apiSecret = exports.cloudinaryApiSecret.value();
    if (!cloudName || !apiKey || !apiSecret) {
        throw new Error('Cloudinary credentials not set. Check .env file in functions directory.');
    }
...
function resolveCloudinaryDeliveryTypeForPrivacy(privacy) {
    const normalized = (privacy || '').trim().toLowerCase();
    if (normalized !== 'private')
        return 'upload';
    const configured = (exports.cloudinaryPrivateDelivery.value() || '').trim().toLowerCase();
    return configured === 'authenticated' ? 'authenticated' : 'upload';
}
function inferCloudinaryDeliveryTypeFromUrl(url) {
...
async function uploadVideoToCloudinary(filePathOrUrl, videoId, privacy) {
    configureCloudinary();
    ...
    const upload = await cloudinary_1.v2.uploader.upload(filePathOrUrl, {
        resource_type: 'video',
...
function extractPublicIdFromUrl(url) {
    ...
    try {
        ...
        if (parts.length === 0 || urlWithoutParams.trim() === '')
            return null;
 ...
 */
function generateSignedCloudinaryUrl(publicId, expirationSeconds = 3600, deliveryType = 'upload') {
    configureCloudinary();
    if (!publicId) {
