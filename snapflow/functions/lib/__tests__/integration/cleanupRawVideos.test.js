...
let videoDocs = {};
let storageFiles = [];
const mockAnalyticsAdd = jest.fn(async (payload) => {
    analyticsWrites.push(payload);
});
const makeTimestamp = (msAgo) => ({
    toMillis: () => Date.now() - msAgo,
});
jest.mock('firebase-admin', () => {
    const buildQuery = (filters = []) => ({
        where: (field, op, value) => buildQuery([...filters, { type: 'where', field, op, value }]),
        orderBy: (field) => buildQuery([...filters, { type: 'orderBy', field }]),
        limit: (count) => buildQuery([...filters, { type: 'limit', value: count }]),
        startAfter: (_doc) => buildQuery(filters),
        get: async () => {
...
const cleanupRawVideos_1 = require("../../storage/cleanupRawVideos");
describe('cleanupRawVideos', () => {
    ...
    const buildFile = (name, size = 1000) => ({
        name,
    ...
    test('deletes failed videos older than 3 days', async () => {
        ...
        storageFiles = [file];
        await (0, cleanupRawVideos_1.runCleanupRawVideos)();
...
//# sourceMappingURL=cleanupRawVideos.test.js.map