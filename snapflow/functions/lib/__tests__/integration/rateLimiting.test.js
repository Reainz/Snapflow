...
describe('Rate Limiting - Integration Tests', () => {
    ...
    let mockDocRef;
    let mockTransaction;
    let transactionState; // Internal state for transaction - shared across all transactions
    // Test user IDs
    ...
    describe('Upload Rate Limiting (5/hour)', () => {
        test('should allow 5 uploads within limit', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                const result = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
                results.push(result);
            ...
            const finalResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
    ...
    describe('Comment Rate Limiting (20/hour)', () => {
        test('should allow 20 comments within limit', async () => {
            // Perform 20 comments
            for (let i = 0; i < 20; i++) {
                const result = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'comment');
                expect(result.allowed).toBe(true);
            }
            // 21st comment should fail
            const finalResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'comment');
            expect(finalResult.allowed).toBe(false);
            expect(finalResult.remaining).toBe(0);
        });
    ...
    describe('Like Rate Limiting (100/hour)', () => {
        test('should allow 100 likes within limit', async () => {
            // Perform 100 likes
            for (let i = 0; i < 100; i++) {
                const result = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'like');
                expect(result.allowed).toBe(true);
            }
            // 101st like should fail
            const finalResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'like');
            expect(finalResult.allowed).toBe(false);
            expect(finalResult.remaining).toBe(0);
        });
    ...
    describe('Follow Rate Limiting (30/hour)', () => {
        test('should allow 30 follows within limit', async () => {
            // Perform 30 follows
            for (let i = 0; i < 30; i++) {
                const result = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'follow');
                expect(result.allowed).toBe(true);
            }
            // 31st follow should fail
            const finalResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'follow');
            expect(finalResult.allowed).toBe(false);
            expect(finalResult.remaining).toBe(0);
        });
    ...
    describe('Flag Rate Limiting (10/day)', () => {
        test('should allow 10 flags within daily limit', async () => {
            // Perform 10 flags
            for (let i = 0; i < 10; i++) {
                const result = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'flag');
                expect(result.allowed).toBe(true);
            }
            // 11th flag should fail
            const finalResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'flag');
            expect(finalResult.allowed).toBe(false);
            expect(finalResult.remaining).toBe(0);
        });
    ...
    describe('Multiple Actions Independence', () => {
        test('should track different actions independently', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
            }
            // Upload should be blocked
            const uploadResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
            expect(uploadResult.allowed).toBe(false);
            // But comment should still work
            const commentResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'comment');
        ...
        test('should allow different actions to have different limits', async () => {
            ...
            expect((await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload')).allowed).toBe(false);
            expect((await (0, rate_limiter_1.checkRateLimit)(testUserId, 'comment')).allowed).toBe(false);
            expect((await (0, rate_limiter_1.checkRateLimit)(testUserId, 'like')).allowed).toBe(false);
            // But follow should still work
            const followResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'follow');
    ...
    describe('End-to-End Flow: Upload to Playback with Rate Limiting', () => {
        test('should allow video uploads within limit', async () => {
            ...
            // 6th upload should fail
            const sixthUpload = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
        ...
        test('should block uploads after limit and show retry message', async () => {
            ...
            // 6th upload attempt
            const blockedResult = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
    ...
    describe('End-to-End Flow: Social Interactions with Rate Limiting', () => {
        test('should allow likes within limit, then block', async () => {
            ...
            // 101st like should be blocked
            const blockedLike = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'like');
        ...
        test('should allow comments within limit, then block', async () => {
            ...
            // 21st comment should be blocked
            const blockedComment = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'comment');
        ...
        test('should allow follows within limit, then block', async () => {
            ...
            // 31st follow should be blocked
            const blockedFollow = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'follow');
    ...
    describe('Storage Cleanup on Rate Limit Exceeded', () => {
        test('should verify upload rejection workflow', async () => {
            ...
            const rejectedUpload = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
        ...
        test('should allow uploads after bucket reset', async () => {
            ...
            // 6th should fail
            const failedUpload = await (0, rate_limiter_1.checkRateLimit)(testUserId, 'upload');
