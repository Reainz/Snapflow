...
describe('processVideoUpload - Storage Cleanup on Rate Limit', () => {
  ...
  let processVideoUpload: any;
  
  beforeAll(async () => {
    // Import after mocks are set up (module caching happens at describe level)
    const module = await import('../../video/processVideoUpload');
  ...
  test('should delete file from Storage when rate limit exceeded', async () => {
    ...
    const mockEvent: CloudEvent<StorageObjectData> = {
      specversion: '1.0',
      id: 'test-event-123',
  ...
  test('should delete files for multiple consecutive rate-limited uploads', async () => {
    ...
    const events = [
      {
        ...
        id: 'test-event-201',
        source: 'storage.googleapis.com',
        type: 'google.cloud.storage.object.v1.finalized' as const,
        time: new Date().toISOString(),
        data: {
    ...
    expect(mockUpdateVideoStatus).toHaveBeenNthCalledWith(
      1,
      'video501',
      expect.objectContaining({ status: 'failed', errorCode: 'RATE_LIMIT_EXCEEDED' })
    );
    expect(mockUpdateVideoStatus).toHaveBeenNthCalledWith(
      2,
      'video502',
