...
describe('Rate Limiter - Unit Tests', () => {
  ...
  let mockDocRef: any;
  let mockDb: any;
  let transactionState: any; // Internal state for transaction
  ...
  describe('Successful Rate Limit Checks', () => {
    ...
    test('should allow first upload within limit', async () => {
      ...
      const result = await checkRateLimit('testUser123', 'upload');
  ...
  describe('Rate Limit Exceeded', () => {
    ...
    test('should calculate correct retry time', async () => {
      ...
      const result = await checkRateLimit('testUser123', 'comment');
  ...
  describe('Different Actions', () => {
    ...
    test('should apply correct limits for each action type', async () => {
      const actions: RateLimitAction[] = ['upload', 'comment', 'like', 'follow', 'share', 'flag'];
      ...
      for (const action of actions) {
        ...
        const result = await checkRateLimit('testUser123', action);
        expect(result.allowed).toBe(true);
    ...
    test('should use daily bucket for flag action', async () => {
      ...
      const result = await checkRateLimit('testUser123', 'flag');
  ...
  describe('Concurrent Requests', () => {
    ...
    test('should handle concurrent rate limit checks with transactions', async () => {
      ...
      let callCount = 0;
      mockTransaction.get.mockImplementation(async (docRef: any) => {
        callCount++;
        const docId = docRef.id;
      ...
      const promises = Array(3).fill(null).map(() => 
        checkRateLimit('testUser123', 'upload')
      );
