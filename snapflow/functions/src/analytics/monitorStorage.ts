...
interface CloudinaryUsageSummary {
...
async function fetchCloudinaryUsageSummary(): Promise<CloudinaryUsageSummary | null> {
  try {
    configureCloudinary();
  } catch (configError) {
  ...
  try {
    ...
    const resourcesSection =
      usageRoot.resources ?? usageRoot.objects ?? usage?.resources ?? usage?.objects ?? {};
 ...
 */
export const monitorStorageUsage = onSchedule({ schedule: '0 2 * * *', timeZone: 'UTC' }, async () => {
    ...
    const bucketStats: StorageStats = {
    ...
    try {
      ...
      const bucketStatsGB: Record<string, { count: number; sizeGB: string; sizeMB: string }> = {};
      ...
      const bytesToGB = (bytes: number) => parseFloat((bytes / (1024 ** 3)).toFixed(3));
      const cloudinaryMetrics = cloudinaryUsage
        ? {
            ...
            bandwidthLimitGB:
              cloudinaryUsage.bandwidthLimitBytes !== null
      ...
      const alerts: Array<{ severity: string; message: string; threshold: number }> = [];
      ...
      if (cloudinaryUsage) {
        const percentOf = (usageBytes: number, limitBytes: number | null) => {
          if (!limitBytes || limitBytes <= 0) return null;
          return (usageBytes / limitBytes) * 100;
        };
        const bandwidthPercent = percentOf(
          cloudinaryUsage.bandwidthBytes,
          cloudinaryUsage.bandwidthLimitBytes
        );
        const storagePercent = percentOf(
          cloudinaryUsage.storageBytes,
          cloudinaryUsage.storageLimitBytes
        );

        if (bandwidthPercent && bandwidthPercent >= 95) {
          alerts.push({
            severity: 'critical',
            message: `Cloudinary bandwidth usage at ${bandwidthPercent.toFixed(
              1
            )}% of plan limit`,
      ...
      for (const [key, stats] of Object.entries(bucketStatsGB) as [string, { count: number; sizeGB: string; sizeMB: string }][]) {
        console.log(`  ${key}: ${stats.count} files, ${stats.sizeGB} GB`);
      }
      if (cloudinaryMetrics) {
        console.log(
          `Cloudinary Bandwidth (30d): ${cloudinaryMetrics.bandwidthGB} GB${
            cloudinaryMetrics.bandwidthLimitGB
              ? ` of ${cloudinaryMetrics.bandwidthLimitGB} GB`
              : ''
          }`
        );
        console.log(
          `Cloudinary Storage: ${cloudinaryMetrics.storageGB} GB${
            cloudinaryMetrics.storageLimitGB ? ` of ${cloudinaryMetrics.storageLimitGB} GB` : ''
          }`
        );
        console.log(
          `Cloudinary Requests (30d): ${cloudinaryMetrics.requests ?? 'unknown'} | Resources: ${
            cloudinaryMetrics.resourceCount ?? 'unknown'
          }`
        );
      } else {
 ...
 */
export const collectCDNMetrics = onSchedule({ schedule: '30 2 * * *', timeZone: 'UTC' }, async () => {
    ...
    const bucketName = `${projectId}.appspot.com`; // Firebase Storage default bucket
    ...
    try {
      ...
      let peakBandwidthTime: Date | null = null;
