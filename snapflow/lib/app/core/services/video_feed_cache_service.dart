...
class VideoFeedCacheService extends GetxService {
  ...
  Future<void> cacheVideo(VideoModel video) async {
    ...
    try {
      ...
      unawaited(
        _persistentCache
            .saveVideo(video, ttl: const Duration(hours: 24))
            .catchError((e, _) {
          Get.log('[VideoFeedCacheService] Persistent cache save failed: $e', isError: true);
        }),
      );
    } finally {
      _releaseLock();
  ...
  void _evictLRU() {
    ...
    unawaited(
      _persistentCache.deleteVideo(lruId).catchError((e, _) {
        Get.log('[VideoFeedCacheService] Persistent cache delete failed: $e', isError: true);
      }),
    );
  }
