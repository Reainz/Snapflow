...
class ProfileController extends GetxController {
  ...
  StreamSubscription<QuerySnapshot<Map<String, dynamic>>>? _userVideosStreamSubscription;
  ...
  // Track the last realtime "first page" ids so we can cheaply detect changes.
  List<String> _lastRealtimeFirstPageIds = const [];
  ...
  Future<void> _loadInitial() async {
    ...
    _setupRealtimeUserVideosUpdates();
    Get.log(
        ...
        .listen(
          (updatedUser) {
            if (updatedUser != null) {
              ...
              final currentUid = _authService.currentUser.value?.uid;
              final wasOwn = isOwnProfile.value;
              isOwnProfile.value = currentUid != null && currentUid == targetUserId;
              ...
              // If profile context changed (own vs other), refresh realtime videos query.
              if (wasOwn != isOwnProfile.value) {
                _setupRealtimeUserVideosUpdates();
              }
            }
          },
  ...
  Query<Map<String, dynamic>> _userVideosQuery(String userId) {
  ...
  void _setupRealtimeUserVideosUpdates() {
    if (targetUserId.isEmpty) return;

    _userVideosStreamSubscription?.cancel();
    _lastRealtimeFirstPageIds = const [];

    try {
      _userVideosStreamSubscription = _userVideosQuery(targetUserId).snapshots().listen(
        (snapshot) {
          final firstPage = snapshot.docs
              .map((d) => VideoModel.fromJson({...d.data(), 'id': d.id}))
              .toList(growable: false);
          final firstPageIds = firstPage.map((v) => v.id).toList(growable: false);

          // If the latest page didn't change, avoid mutating the observable list.
          if (_lastRealtimeFirstPageIds.length == firstPageIds.length) {
            var same = true;
            for (var i = 0; i < firstPageIds.length; i++) {
              if (_lastRealtimeFirstPageIds[i] != firstPageIds[i]) {
                same = false;
                break;
              }
            }
          ...
          _lastRealtimeFirstPageIds = firstPageIds;

          // Merge newest realtime page with any already-paginated items.
          final existing = userVideos.toList(growable: false);
          final firstPageIdSet = firstPageIds.toSet();
          final merged = <VideoModel>[
            ...firstPage,
            ...existing.where((v) => !firstPageIdSet.contains(v.id)),
          ];
          userVideos.assignAll(merged);

          // Keep the counter in sync with backend visibility rules.
          unawaited(_updateVisibleVideosCount());
        },
        onError: (e) {
          if (LogConfig.verbose) {
            Get.log('Realtime user videos subscription error: $e', isError: true);
          }
        },
      );
    } catch (e) {
      if (LogConfig.verbose) {
        Get.log('Failed to start realtime user videos subscription: $e', isError: true);
      }
    }
  }
