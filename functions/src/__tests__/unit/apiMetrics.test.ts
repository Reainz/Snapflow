...
describe('collectAPIMetrics', () => {
  ...
  let mockFirestore: jest.Mock;
  let mockListTimeSeries: jest.Mock;
  ...
  beforeEach(() => {
    ...
    firestoreSpy = jest.spyOn(admin, 'firestore').mockImplementation(mockFirestore as any);
    
    // Also add the static properties to the spy itself (for direct property access)
    (firestoreSpy as any).FieldValue = mockFieldValue;
    (firestoreSpy as any).Timestamp = mockTimestamp;
    ...
    (MetricServiceClient as unknown as jest.Mock).mockImplementation(() => mockMonitoringClient);
  });
  ...
  describe('MetricServiceClient Integration', () => {
    test('should initialize MetricServiceClient', async () => {
      ...
      expect(MetricServiceClient).toHaveBeenCalled();
    });
    ...
    test('should query execution_count metric for each function', async () => {
      ...
      const executionCountCalls = mockListTimeSeries.mock.calls.filter((call: any) => 
        call[0].filter.includes('execution_count')
      );
    ...
    test('should query error_count metric for each function', async () => {
      ...
      const errorCountCalls = mockListTimeSeries.mock.calls.filter((call: any) => 
        call[0].filter.includes('error_count')
      );
    ...
    test('should query execution_times metric for average and p95', async () => {
      ...
      const executionTimesCalls = mockListTimeSeries.mock.calls.filter((call: any) => 
        call[0].filter.includes('execution_times')
      );
  ...
  describe('Firestore Write Verification', () => {
    ...
    test('should include all monitored functions in metrics', async () => {
      ...
      const functionNames = new Set(savedDoc.functions.map((f: any) => f.name));
  ...
  describe('Error Handling', () => {
    ...
    test('should create admin alert on failure', async () => {
      ...
      const alertCalls = mockCollection.mock.calls.filter((call: any) => 
        call[0] === 'admin_alerts'
      );
      expect(alertCalls.length).toBeGreaterThan(0);

      // Verify alert structure
      const alertDoc = mockAdd.mock.calls.find((call: any) => 
        call[0]?.type === 'api_metrics_error'
      )?.[0];
  ...
  describe('Edge Cases', () => {
    test('should handle functions with zero invocations', async () => {
      ...
      savedDoc.functions.forEach((func: any) => {
  ...
  describe('Data Structure Validation', () => {
    ...
    test('should round floating point values appropriately', async () => {
      ...
      const funcWithTime = savedDoc.functions.find((f: any) => f.avgResponseTimeMs > 0);
      if (funcWithTime) {
      ...
      const funcWithErrors = savedDoc.functions.find((f: any) => f.errorRate > 0);
      if (funcWithErrors) {
