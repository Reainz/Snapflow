...
export const generateSignedUrl = onCall(async (request) => {
  ...
  try {
    ...
    if (isCloudinaryUrl(hlsUrl)) {
      ...
      try {
        // Only "private" videos use Cloudinary authenticated delivery.
        // "followers-only" access is enforced via Firestore privacy rules.
        const isPrivate = videoData.privacy === 'private';
