...
export async function uploadVideoToCloudinary(
  ...
  configureCloudinary();
  // Treat only explicit "private" videos as Cloudinary-authenticated assets.
  // "followers-only" access is enforced at the Firestore / API layer instead,
  // so those videos can use standard public delivery for simpler playback.
  const isPrivate = privacy === 'private';
  const deliveryType: 'upload' | 'authenticated' = isPrivate ? 'authenticated' : 'upload';
  const accessMode = isPrivate ? 'authenticated' : 'public';
