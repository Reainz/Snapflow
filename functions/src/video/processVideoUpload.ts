...
export const processVideoUpload = onObjectFinalized(
  ...
  // Mark uploader as active (so DAU/WAU reflect uploads even if clients haven't rolled out lastActiveAt updates yet).
  try {
    ...
    logger.warn('Failed to update user activity timestamps', {
      userId,
      videoId,
      error: err?.message || String(err),
    });
  ...
  // Ensure the video doc has an ownerId and createdAt (some code paths create docs from the storage trigger first).
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      tx.set(videoRef, payload, { merge: true });
    });
  } catch (err: any) {
    logger.warn('Failed to upsert video ownership metadata', {
      userId,
      videoId,
      error: err?.message || String(err),
    });
